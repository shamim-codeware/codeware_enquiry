                    ...
                    <div class="col-lg-12">
                        <div class="card card-default card-md mb-4">
                            <div class="card-header">
                                <h6>Update Enquiry Source</h6>
                            </div>
                            <div class="card-body py-md-30">
                                <form action="{{url('enquiry-source/'.$enquirySource->id)}}" method="post">
                                    @method('put')
                                    @csrf 
                                            ...
                                            <div class="mb-25 select-style2">
                                                <div class="dm-select ">
                                                    <select  name="parent_id" id="select-parent-source" class="form-control ">
                                                        ...
                                                        <option value="{{ $enquirySource->parent_id }}">{{ $enquirySource->parents ? $enquirySource->parents->name : ''}}</option>
                                                        @foreach($enquiryparents as $enquiryparent)
                                                            <option value="{{ $enquiryparent->id }}" {{ $enquirySource->parent_id && $enquirySource->parent_id == $enquiryparent->id? 'selected': ''}} >{{ $enquiryparent->name  }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
